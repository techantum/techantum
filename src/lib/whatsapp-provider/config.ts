...
export function getMetaProviderConfig() {
  ...
  return {
    ...
    systemUserAccessToken: env('META_SYSTEM_USER_ACCESS_TOKEN') || env('META_WHATSAPP_ACCESS_TOKEN'),
    businessId: env('META_BUSINESS_ID') || env('META_WHATSAPP_BUSINESS_ACCOUNT_ID'),
...
export function getPublicMetaSignupConfig() {
  ...
  return {
    ...
    configured: Boolean(cfg.publicAppId && cfg.embeddedSignupConfigId && cfg.graphVersion && cfg.appSecret),
  };
}
